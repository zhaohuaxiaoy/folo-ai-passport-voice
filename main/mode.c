...
{
    return (m == APP_MODE_WIFI) ? "WiFi" : "BLE";
}
...
{
    audio_streamer_set_sender(s_mode == APP_MODE_WIFI ? send_audio_wifi : send_audio_ble);
}
...
esp_err_t mode_init(void)
{
    ...
    s_mode = (m == 0) ? APP_MODE_BLE : APP_MODE_WIFI;
...
esp_err_t mode_switch(app_mode_t target)
{
    ...
    app_event_t d = { .type = (s_mode == APP_MODE_WIFI)
                               ? APP_EV_WS_DISCONNECTED : APP_EV_BLE_DISCONNECTED };
    app_event_post(&d);
