...
// 窗口(voice.end drain ≤500ms / cancel ≤200ms),与高突发源(转写/CTRL 行)
// 时间正交;满即丢+日志兜底不变,关键收束事件走 post_important 阻塞投递
// 不丢;被丢的最坏是 display-only 转写行(逐行覆盖自愈,日志可观测)。
// 注:TIME_SET 引入 int64 后结构 240B(8 深 = 1920B),上述分析不变(长度无关)。
