...
- **按住说话**：READY 态按住**音量+**立即开始录音（按下即录，滴声提示），松开自动发送；无取消窗口、无超时残留。短于 500ms 的误碰当场收束丢弃，不进转写
...
> 长按阈值：三颗键统一 500ms —— DOWN 长按清空、OK 长按锁屏/解锁（锁定 = 省电息屏，按键照常执行但不亮屏）。UP 不用阈值开录（**按下即录**，2026-08-29：原先等 0.5s 判定 + 等滴声播完才开流，真机实测按下到实际录音 ~1.01s，第一秒的话被吞掉），阈值在 UP 上只划定"这一按事后还会不会补报单击"的边界，与固件的 `PTT_MIN_TALK_MS` 同值。
>
> 手势排布规则（2026-08-29 定稿）：同一颗键上只放「单击 + 长按」。iot_button 一旦判成长按就不再补报单击/双击，两者天然互斥；而「单击 + 双击」必须等双击窗口才敢执行单击（迟滞或误发），「长按 + 双击」最糟——长按到点就宣布，直接把双击判死。清空因此从「UP 双击」改到「DOWN 长按」。
