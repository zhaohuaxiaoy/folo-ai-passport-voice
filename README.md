...
| 层 | 组件 | 职责 |
| --- | --- | --- |
| 设备端 | ESP32-C3 固件（ESP-IDF 5.5 + LVGL 9.5） | 状态机、按键/录音、UI 渲染、三通道传输、功耗管理 |
| 桌面端 | companion（macOS / Windows） | 三通道接入、火山流式 ASR 转发、悬浮窗、剪贴板注入、向导、托盘 |
| 云端 | 火山引擎大模型流式 ASR | 音频 → 文本（partial 全量累积 / final 定稿） |

数据流：设备采集 16kHz 音频 → 按 100ms 一帧经 **BLE / WiFi+WS / USB** 任一通道上行 → 桌面端 relay 流式送入火山 ASR → 中间结果实时回显到悬浮窗与设备屏幕 → 松手定稿后注入一次、收窗。反向通道承载 Agent 状态、审批请求与按键裁决。

## 项目亮点

- **三通道冗余链路**：一套音频/事件协议跑三条物理通道——BLE（Mac 直连，GATT 服务 `0xA2B0`）、WiFi+WebSocket+mDNS（无蓝牙 PC，自动发现自动重连）、USB-Serial-JTAG（有线调试 + 完整控制台命令面）。任意通道断线自动收束回 READY、审批保持等待重连，链路故障不会废掉设备。
...
- **三通道传输**：BLE / WiFi+WS+mDNS / USB-Serial-JTAG（含完整控制台命令面）
...
- **为什么三通道**：BLE 只覆盖有蓝牙的 Mac；Windows 无蓝牙 PC 走 WiFi+WS（mDNS 自动发现）；USB 是调试器兼最后保底——任何单一链路失效都可用另一条继续工作，断线事件统一收束，不残留半开会话。
...
语音输入是这条管道上的**第一个应用**：同一套音频流 + 事件协议（三通道共用）天然可以承载更多能力——转写、翻译、Agent 指令、远程控制。固件与桌面端均在 MIT 许可下开源，换壳、加页面、接新服务都从干净的架构边界开始。
