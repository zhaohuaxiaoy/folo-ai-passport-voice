...
当前仓库已经公开固件源码、BSP API、引脚/器件定义、已知硬件陷阱、应用示例和验收方法；这足以让 agent 在**现有 BSP 能力范围内**开发应用。

但仓库目前没有根目录软件/硬件许可证，也没有原理图、可编辑 PCB、BOM、Gerber、装配说明、板卡修订历史和完整器件资料。因此，严格来说它目前是一个开放的固件与硬件开发基线，尚不是可以独立制造、修改和分发的完整开放硬件发布包。

要补齐这一层，建议后续增加：

- `hardware/` 中的可编辑原理图与 PCB 工程，以及可直接查看的 PDF；
- BOM、Gerber/钻孔文件、坐标文件、装配图和生产注意事项；
- 板卡版本、变更记录、测试点、电源树和所有连接器/未用 GPIO 定义；
- LCD 模组、电池、充电/电源、扬声器、麦克风与功放的准确型号和限制；
- 明确的 OSHW 许可证、软件许可证及第三方素材/依赖的归属；
- 每个硬件版本的 bring-up 日志、测试固件、测量范围和已知问题。

资料补齐后，应同步更新 `bsp_pins.h` 和 AI 硬件开发指南，使“设计文件—BSP—示例—验收证据”形成同一条可追溯链路。这才是本项目所追求的 agent-native 开放硬件：agent 不只会生成代码，也知道哪些事实可信、哪些资源有限、如何验证，以及什么时候必须停止猜测。
